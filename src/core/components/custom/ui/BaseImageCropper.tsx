  ...
  useEffect(() => {
    if (imageWrapperWidth > 0 && imageWrapperHeight > 0 && cropAreaWidth > 0 && cropAreaHeight > 0) {
      ...
      if (!isInitialSetupDoneRef.current) {
        ...
        if (onCropChange)
          onCropChange(calculateCropData(restrictedInitial.x, restrictedInitial.y, currentZoomForSetup));
        ...
        if (onCropChange)
          onCropChange(calculateCropData(restrictedCurrent.x, restrictedCurrent.y, currentZoomForSetup));
      }
    } else {
      isInitialSetupDoneRef.current = false;
      setOffsetX(0);
...
> = ({ className: className, ...restProps }) => {
  ...
  return (
    <div style={getImageWrapperStyle()}>
      <img {...imageProps} alt={'img-croper'} className={className} {...restProps} />
    </div>
  );
};
